/* Toddler Music Box — frontend (HTML/JS + Web Audio API)
  * 1. Start screen: the first tap starts the AudioContext (required by iOS/Chrome)
  ...
  * 8. Quiz: 3 random notes from the selected clef, answered on a MIDI keyboard / real piano / on-screen keys
...
(() => {
  ...
  const idToMidi = (id) => PC.indexOf(id.slice(0, -1)) + 12 * (+id.slice(-1) + 1);
  const midiToId = (m) => PC[m % 12] + (Math.floor(m / 12) - 1);   // 60 → "C4" (middle C)
  ...
  function buildKeys() {
    ...
    whiteEls = WHITE_SLOTS.map((k) => {
      ...
      // Heart and arrows are SVG, other shapes are CSS; below the shape is the note on the staff (treble or bass)
      const svg = SHAPES_SVG[shape]
        ? `<svg viewBox="0 0 100 100" aria-hidden="true"><path d="${SHAPES_SVG[shape]}"/></svg>` : "";
      b.innerHTML = `<span class="shape" aria-hidden="true">${svg}</span>` +
                    `<img class="key-staff" src="staff/${clef}/${note}.svg" alt="" draggable="false">`;
      b._info = k;
  ...
  function fallbackTone(note) {
    const freq = 440 * 2 ** ((idToMidi(note) - 69) / 12);
  ...
  function advanceSong(note) {
    ...
    if (songPos >= curNotes.length) {              // end of song
      ...
      songPos = 0;
      if (listening) {                               // finished on a real piano → stop listening, show "Great job"
        stopPiano();
        pianoStatus.textContent = `🎉 ${T.great}`;
      }
    }
    highlightNext();
  }
  ...
  function onPianoNote(midi, source) {
    let id = midiToId(midi);
    ...
    if (source === "mic") {
      ...
      const pc = PC[midi % 12];
      id = target && target.slice(0, -1) === pc ? target : pc + baseOct();
    }
  ...
  async function startPiano() {
    // 1. If a MIDI keyboard is connected, use it — no microphone needed
    if (await PianoInput.startMidi(onPianoNote) > 0) return beginListening("midi");
    // 2. Check mic permission: granted → start; otherwise show our explanation dialog first
  ...
  micClose.addEventListener("click", closeMicDialog);

  function stopPiano(keepMessage) {
    listening = false;
    PianoInput.stop();
    pianoBtn.classList.remove("active");
    pianoBtn.textContent = `🎹 ${T.piano}`;
    if (!keepMessage) { pianoState = ""; pianoStatus.textContent = ""; }
  }

  pianoBtn.addEventListener("click", () => (listening ? stopPiano() : startPiano()));
  ...
  function startQuiz() {
    ...
    quiz = { notes: pickQuizNotes(), pos: 0, wrong: 0, ownListening: !listening };
    ...
    showQuizTarget();
    if (!listening) startPiano();                   // also listen to a MIDI keyboard / real piano (mic asks first)
  }

  function endQuiz() {
    const own = quiz && quiz.ownListening;
    quiz = null;
    clearTimeout(revealKey.t);
  ...
  document.getElementById("startBtn").addEventListener("click", () => {
    const fs = enterFullscreen();                        // synchronous call (inside the user gesture)
    const audio = initAudio();                           // the AudioContext must also be created inside the gesture
    startEl.classList.add("hidden");
    fs.then(lockLandscape);
    if (!started) { started = true; trapBack(); }
    audio.then(() => play(noteOf(WHITE_SLOTS[0])));
  });

  // If swiped out of fullscreen: cover with a "Tap to continue" screen that re-enters fullscreen
  const onFsChange = () => {
    if (!fsElement() && started && !isInstalled()) {
      startHint.textContent = T.continue;
