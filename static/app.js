/* Hear-a-Note for Toddlers — frontend (HTML/JS + Web Audio API)
...
(() => {
  ...
  function applyLang() {
    ...
    document.title = T.appName;                     // one language only (Hear-a-Note for Toddlers, or the local name)
  ...
  // 1.2 x the key's shape, centered, its bottom 45 % of its height above the key's bottom (never over the black keys); plays to the end, about 3 s,
  ...
  function confetti(add, W) {                        // W = width of the effect box in px (1 % = W / 100)
    const cq = (n) => (n * W / 100).toFixed(1) + "px";
    for (let i = 0; i < 18; i++) {
      const a = -Math.PI / 2 + (Math.random() - .5) * 2.4;          // mostly upwards, like a little fountain
      const r = 26 + Math.random() * 22;
      add({ "--dx": cq(Math.cos(a) * r), "--dy": cq(Math.sin(a) * r * .9),
            "--fall": cq(34 + Math.random() * 20), "--r": (Math.random() * 540 - 270) + "deg",
  ...
  function reward(note) {
    ...
    // The staff note is 1.2 x the key's shape, centered on the key, its bottom 45 % of its own height above the key's bottom edge,
    ...
    if (shapeEl && !key.classList.contains("black")) {
      ...
      const room = (keyBottom - blackBottom - 4) / 1.45;           // white part below the black keys (incl. the 45 % gap)
      img = Math.min(shapeEl.offsetWidth * 1.2, room);
      cx = key.offsetLeft + key.offsetWidth / 2;
      cy = keyBottom - img * 0.45 - img / 2;                        // its bottom is 45 % of its own height above the key's bottom
    ...
    box.style.height = w + "px";                                    // (square; older Safari has no aspect-ratio)
    box.style.setProperty("--cqw", w / 100 + "px");
  ...
  function cardReward(scale) {
    ...
    const cw = (shape.offsetWidth * Math.max(scale, 1)) / 0.62;
    box.style.width = cw + "px";
    box.style.height = cw + "px";
    box.style.setProperty("--cqw", cw / 100 + "px");
  ...
  function bubbleParty() {
    ...
    const AW = app.clientWidth / 100, AH = app.clientHeight / 100;   // 1 % of the app (px; works on older Safari too)
    for (let i = 0; i < COLS * ROWS; i++) {
      ...
      b.style.setProperty("--s", s * AH + "px");
      b.style.setProperty("--x", Math.min(94, Math.max(6, x)) * AW + "px");
      b.style.setProperty("--y", Math.min(90, Math.max(10, y)) * AH + "px");
  ...
  if (window.ResizeObserver) new ResizeObserver(layoutBlackKeys).observe(keysEl);
  else window.addEventListener("resize", () => setTimeout(layoutBlackKeys, 60));
  ...
  let silentEl = null;
  // Older Safari: play() and resume() may return nothing instead of a Promise
  ...
  function unlockAudio() {
    ...
    if (IS_IOS && !silentEl) {
      ...
      const pp = silentEl.play();
      if (pp && pp.catch) pp.catch(() => { silentEl = null; });   // try again on the next tap
    }
    resumeCtx();
  }
  ...
  function playKeepAwakeVideo() {
    ...
    if (keepVideo.paused) quiet(keepVideo.play());
  }
