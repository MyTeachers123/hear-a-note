...
- The shape on the current note card and the same shape on the right key do a slow, soft "jelly" wiggle together (every 4 seconds), so the child sees which key to press. Tapping the card plays that note.
